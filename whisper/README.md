...
docker run --rm -v "$(pwd)/models:/models" -e "http_proxy=$http_proxy" -e "https_proxy=$https_proxy" allape/whisper:latest bash -c "/app/models/download-ggml-model.sh base /models"
docker compose up -d
